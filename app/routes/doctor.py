"""Doctor dashboard: request a patient's record. Denied if no active grant."""
import json
...
def dashboard():
    user = session["user"]
    patients = [u for u in cognito_client.list_users() if u["role"] == "patient"]
...
def request_record():
    ...
    if r["status"] == 200:
        ...
        flash(f"Record retrieved (audit block #{r['body']['block']}).", "info")
    else:
